...
int makeMaxSegmentTree(int start, int end, int node)
{
	int mid;

	if (start == end)
	{
		return maxSegmentTree[node] = arr[start];
	}

	mid = (start + end) / 2;
	return maxSegmentTree[node] = max(makeMaxSegmentTree(start, mid, node * 2),
		makeMaxSegmentTree(mid + 1, end, node * 2 + 1));
}
